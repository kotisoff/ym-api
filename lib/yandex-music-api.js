...
export default class YandexMusicApi {
  ...
  init(config) {
    ...
    return this._performRequest(rest.post.bind(rest), request).then(function (data) {
      ...
      return this._performRequest(rest.post.bind(rest), request).then(function (token) {
  ...
  getUserPlaylists(userId) {
    const request = _get()
      .setPath('/users/' + (userId || this.#config.user.UID) + '/playlists/list')
      .addHeaders(this._getAuthHeader());
  ...
  getPlaylist(userId, playlistKind) {
    const request = _get()
      .setPath('/users/' + (userId || this.#config.user.UID) + '/playlists/' + playlistKind)
      .addHeaders(this._getAuthHeader());
  ...
  removePlaylist(playlistKind) {
    const request = _get()
      .setPath('/users/' + this.#config.user.UID + '/playlists/' + playlistKind + '/delete')
      .addHeaders(this._getAuthHeader());
  ...
  renamePlaylist(playlistKind, name) {
    const request = _get()
      .setPath('/users/' + this.#config.user.UID + '/playlists/' + playlistKind + '/name')
      .addHeaders(this._getAuthHeader())
      .setBodyData({
        'value': name
      });
  ...
  addTracksToPlaylist(playlistKind, tracks, revision, options) {
    const opts = options || {};
    const request = _get()
      .setPath('/users/' + this.#config.user.UID + '/playlists/' + playlistKind + '/change-relative')
      .addHeaders(this._getAuthHeader())
      .setBodyData({
        'diff': JSON.stringify([{
          'op': 'insert',
